...
\section{Quick start}
As a quick demonstration of the analysis is largely impossible, this part will just be left empty. 
Suffice to say that there are several components of the pipeline that shall be discussed:
\begin{enumerate}
...
\end{enumerate}
In the various examples for this guide, data will be used from an experiment targeting RNA polymerase II in MCF7 cells \citep{li2012extensive}. 
...
Filtering on the average count (as computed by the aveLogCPM function in the edgeR package \citep{mccarthy2010glm}) is recommended for statistical analyses using a negative binomial (NB) model.
...
This accounts for biases introduced by non-equal proportions of each linker as well as non-random ligation. 
Failure to normalize for these effects will result in incorrect conclusions. 
...
Assume that ligation between DNA fragments is random. 
Each DNA fragment can be treated as a diploid individual where each end represents an allele. 
Random ligation is equivalent to random mating between individuals. Now
, assume that the proportions of DNA fragments attached to linker A and B are $a$ and $b=1-a$ respectively. 
...
Alternatively, normalization can be performed to transform the expected ratio into a simple 1:1 comparison.
In the example above, the expected ratio is 5:4. 
One could then normalize the data by dividing the homo-linker counts by 1.25 prior to any comparison.

\subsection{Checking mitochondrial counts}
The accuracy of the random ligation model can be assessed for each dataset by using \code{extractMito} to count tag pairs mapped between the nuclear and mitochondrial chromosomes.
The assumption here is that the mitochondrial genome does not interact with the nuclear chromosomes. 
...
You can also go through the motions with a Pearson's chi-squared test to confirm this result. 
...
The assumption here is that most bin pairs contain only non-specific ligation events, i.e., most contacts in the interaction space are not specific. 
This is generally reasonable as contacts would not be expected between all pairs of binding sites. 
Again, large bins are necessary to count rare non-specific events.
...
However, the bulk of interactions have a substantially higher M-value. 
...
Different datasets may experience differing violations of the randomness assumption, so the above diagnostics are recommended for quality control.

\section{Normalizing for empirical effects}
\label{sec:norm}
Normalization must account for non-equal linker proportions and non-random ligation. 
This can be done empirically by assuming that most interactions are non-specific. 
This means that the average M-value for most interactions can be used to define the expected homo-/hetero-linker ratio under the non-specific hypothesis. 
For example, if most interactions have an M-value close to 2, one would expect a homo-/hetero-linker ratio of around 4:1. 

Normalization can then be performed to scale the counts according to the expected ratio. 
Specifically, the binned homo-/hetero-linker counts are passed into the trimmed mean of M-values (TMM) procedure \citep{oshlack2010tmm}. 
This  returns a normalization factor for each set of counts in each library. 
Counts are then (conceptually) divided by the normalization factor, such that a direct comparison can be performed between the normalized counts. 
For example, an expected 4:1 ratio would involve dividing the homo-linker counts by 4 prior to comparison.
...
In the MA plot above, all inter-chromosomal interactions lie within the main bulk of M-values at $\sim$2 whereas only intra-chromosomal interactions make up the cloud at M-values of $\sim$7.

\chapter{Assessing biological variability}

\section{Motivation}
Datasets with biological replicates account for biological variability when performing hypothesis testing. 
...
Estimation of the NB dispersion parameter allows modelling of te variation between biological replicates. 
...
The most obvious choice is a paired-sample design, where the homo- and hetero-linker counts from a single library are paired together.
...
This avoids inflation of the dispersions due to batch effects between libraries.
...
The \code{estimateDisp} function will estimate the NB dispersion whereas the \code{glmQLFTest} function will estimate the QL dispersion. 
...
Note that the biological coefficient of variation is defined as the square root of the NB dispersion. 
...
However, this is a fairly arbitrary solution as the estimated dispersion will depend on the chosen prior. 
...
In this author's opinion, the design for routine ChIA-PET analyses should use a one-way layout. 
...
Of course, the one-way layout assumes that there are no library-specific effects. 
For example, if the absolute count size changed randomly between libraries but the homo-/hetero-linker fold change was constant, the dispersions would be inflated in the one-way design but not in the paired-sample design.
...
For most other types of sequencing data (e.g., RNA-seq, ChIP-seq), NB dispersion estimates are usually around 0.01 to 0.1 but much lower values are observed here. 
This is a bit puzzling given that ChIA-PET should reflect the variability of ChIP-seq, and then some.
