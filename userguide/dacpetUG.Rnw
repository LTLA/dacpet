...
\vspace{6pt}
Last revised 20 September 2014}
...
\section{Scope}
This document describes the use of the \pkgname{} package to analyze CHIA-PET data.
In particular, specific interactions are distinguished from non-specific ligation events by comparing the homo-linker and hetero-linker counts for each interaction. 
Analyses can also be performed to identify interactions with significant changes in intensity between biological conditions.
...
Thoughtful suggestions for improvements are occasionally appreciated.
...
Each bin pair consists of one anchor and one target bin (specified in \code{anchor} and \code{target}, respectively), where the anchor bin is that with the higher genomic coordinate.
...
require(edgeR)
pc <- 1e-3
a.cpm <- aveLogCPM(asDGEList(freqs), prior.count=pc)
summary(a.cpm)
@
...
The abundance of larger bin pairs can be calculated and compared to the values in \code{a.cpm}.
Some adjustments are necessary to account for the differences in the \textit{areas} for tag pair collection with each bin size.
The median of the adjusted values is then increased by \code{X} and used as the threshold.

<<>>=
adjusted <- (binsize/countwidth)^2
threshold <- median(aveLogCPM(asDGEList(rebin), prior.count=pc*adjusted))
threshold <- threshold - log2(adjusted)
X <- 5000
keep <- a.cpm >= threshold + log2(X) 
summary(keep)
@
...
<<>>=
inters <- asDGEList(rebin[is.na(getDistance(rebin)),])
inters <- calcNormFactors(inters)
normfacs <- inters$samples$norm.factors
normfacs
@
...
\section{Motivation}
The availability of biological replicates means that the analysis can account for biological variability when performing hypothesis testing. 
...
Power can be improved by filtering out all bin pairs with an inappropriate log-fold change, prior to applying the BH method.
This removes the uninteresting tests and reduces the severity of the FDR correction.
