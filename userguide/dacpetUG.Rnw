...
A more educated choice of filter threshold can be obtained by identifying putative genuine interactions as those that have, e.g., higher average abundance than a non-specific interaction. 
...
This is generally reasonable as chromosomes are organized into self-contained territories within the nucleus.
This sterically limits the potential contacts between binding sites on different chromosomes.
...
That the inter-chromosomal M-values are generally equal to or less than the nuclear-mitochondrial M-values also suggests that most inter-chromosomal interactions are non-specific.
...
points(a[has.mito], m[has.mito], pch=16, cex=0.5, col="red")
@
