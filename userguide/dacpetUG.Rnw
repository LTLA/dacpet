...
Alternatively, the stripped results can be diverted to a new tag file if further quality control on the original results is desired. 
...
The various demonstrations here will use processed tag files for the full MCF7 dataset. 
A little bit of work is necessary to determine which files contain the hetero-linker counts.
Similarly, some effort is required to determine which files are generated from the same library. 
...
libname <- sub("_[AB]+", "", basename(curfiles))
...
This means that only homo-linker tag pairs need to be used (or indeed, are available) for peak calling. 
...
In this case, MACS \citep{zhang2008macs} was called with the command \code{macs14 -t pooled.bed -f BED -n here -{}-nomodel}. 
...
<<>>=
peaked <- recountPET(curfiles, peaks, filter=20L)
head(peaked$counts)
@
...
Note that this refers to each library rather than each tag file, such that each total includes tag pairs from all linker combinations. 
...
Any pedants out there can also go through the motions with a Pearson's chi-squared test to confirm this result. 
...
Making diagnostic plots like those shown above is recommended for routine quality control.
...
In this author's opinion, routine ChIA-PET analyses should use a one-way layout for the design matrix. 
...
\section{Loading for homo-linker counts}
...
Filtering is performed to remove low-abundance features for which detection power will always be low.
...
keep <- aveLogCPM(comp.freq$counts, lib.size=comp.freq$totals) > 0 &
	(min.gap > 1 | is.na(min.gap))
comp.freq$counts <- comp.freq$counts[keep,]
comp.freq$pairs <- comp.freq$pairs[keep,]
summary(keep)
@
...
<<>>=
y.diff <- DGEList(comp.freq$counts, lib.size=comp.freq$totals)
...
inter.counts <- rebin.diff$counts[is.na(getDistance(rebin.diff)),]
normfacs <- calcNormFactors(DGEList(inter.counts, lib.size=rebin.diff$totals))$samples$norm.factors
normfacs
@
...
This maintains any differences in the high-abundance specific interactions.
Otherwise, systematic differences in the specific interactions are technical artifacts.
The first approach should then be used as it will remove such spurious differences.
...
result.diff <- glmQLFTest(y.diff, design.diff, robust=TRUE, plot=TRUE)
@
...
Each group would have a different homo-/hetero-linker fold change.
The null hypothesis would then state that the fold change for each group is equal.

<<>>=
count.origins <- factor(c("A_hom", "A_het", "B_hom", "B_het"))
design.diff2 <- model.matrix(~0+count.origins)
colnames(design.diff2) <- levels(count.origins)
my.contrast <- c(-1, 1, 1, -1)
design.diff2
@

Here, \code{my.contrast} defines the null as \code{B\_hom - B\_het - (A\_hom - A\_het)=0}.
This should avoid detection of any spurious changes in the homo-linker counts that are driven by non-specific ligation.
For example, an increase in \code{B\_hom} over \code{A\_hom} will not be detected if there is a corresponding increase in \code{B\_het} over \code{A\_het}.
...
<<>>=
ax <- getAnchor(comp.freq)
tx <- getTarget(comp.freq)
final <- data.frame(anchor.chr=seqnames(ax), anchor.start=start(ax), anchor.end=end(ax),
    target.chr=seqnames(tx), target.start=start(tx), target.end=end(tx), 
